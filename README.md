...
•	Production Mode: The heavy, server-side execution for biological data analysis.
⚠️  You must never run a Production execution without first passing a Mock test to validate your environment.
...
1.	Download the Pipeline: Place the 2026_multiomnicpipeline directory in an accessible location (e.g., your Desktop).
o	Windows Users: To find your exact path, right-click the folder and select "Copy as path". It should resemble: "C:\Users\YourName\Desktop\2026_multiomnicpipeline".
2.	Reference Genome: Ensure the GRCh38 Human Reference Genome is downloaded to the server. If it is already hosted centrally, obtain the direct directory path to its location.
...
python step0_main_pipeline.py \
  ...
  -t 4 \
  --mode mock
5. Server Execution in Production Mode
